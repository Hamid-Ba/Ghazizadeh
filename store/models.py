...
class Category(models.Model):
    ...
    title = models.CharField(max_length=72, null=False, blank=False, verbose_name="عنوان")
    logo = models.ImageField(null=True, blank=True, upload_to=category_logo_file_path, verbose_name="لوگو")
...
class Product(models.Model):
    ...
    price = MoneyField(
        max_digits=12, decimal_places=0, default_currency="IRR", null=False, verbose_name="قیمت"
    )
    ...
    gallery = models.ManyToManyField(gallery_models.Gallery, related_name="products", verbose_name="گالری")

    brand = models.ForeignKey(
        brand_models.Brand, on_delete=models.CASCADE, related_name="products", verbose_name="برند"
    )
...
class Comment(models.Model):
    ...
    full_name = models.CharField(max_length=125, null=False, blank=False, verbose_name="نام کامل")
...
class PaymentMethod(models.Model):
    ...
    title = models.CharField(max_length=125, null=False, blank=False, verbose_name="عنوان")
    price = MoneyField(
        max_digits=10, decimal_places=0, default_currency="IRR", null=False, verbose_name="هزینه"
    )
...
class Order(models.Model):
    ...
    code = models.CharField(max_length=225, blank=False, null=True, verbose_name="کد سفارش")
    state = models.CharField(
        max_length=2, default=OrderState.PENDING, choices=OrderState.choices, verbose_name="وضعیت"
    )
    total_price = MoneyField(
        max_digits=10, decimal_places=0, default_currency="IRR", null=False, verbose_name="قیمت کل"
    )
    phone = models.CharField(
        max_length=11, blank=False, null=False, validators=[validators.PhoneValidator], verbose_name="موبایل"
    )
    registered_date = models.DateTimeField(
        auto_now_add=True, editable=False, verbose_name="تاریخ ثبت سفارش"
    )
    address = models.ForeignKey(
        address_models.Address, on_delete=models.DO_NOTHING, related_name="orders", verbose_name="نشانی"
    )
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="orders", verbose_name="کاربر"
    )
...
class OrderItem(models.Model):
    product_id = models.BigIntegerField(null=True, blank=True, verbose_name="شناسه محصول")
    brand = models.CharField(max_length=125, null=True, blank=True, verbose_name="برند")
    title = models.CharField(max_length=125, null=True, blank=True, verbose_name="عنوان")
    # image_url = models.CharField(max_length=250, null=False, blank=False)
    price = MoneyField(
        max_digits=10, decimal_places=0, default_currency="IRR", null=True, verbose_name="قیمت"
    )
    technical_number = models.CharField(max_length=125, null=True, blank=True, verbose_name="کد فنی")
...
class FavoriteProduct(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="favorited_by",verbose_name="محصول"
    )
